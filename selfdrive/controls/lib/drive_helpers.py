...
MAX_CURVATURE_RATES_0 = [0.03762194918267951, 0.03762194918267951 * 1.0] #最初の係数を機械推論反映値として計算する（1〜2.7）
...
def get_lag_adjusted_curvature(CP, v_ego, steerAng , psis, curvatures, curvature_rates):
  ...
  MAX_CURVATURE_RATES_k = 1.2 + (2.7 - 1) * abs_sta #1.2〜2.7+0.2
  ...
  max_k_v = 1.9
  max_k_v = 1.1 + (max_k_v - 1) * abs_sta #max_k_v = 1.1〜max_k_v+0.1 , ひとまずハンドル（前方カーブ予測含む）が10度で最大値になる。
